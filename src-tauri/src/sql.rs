...
impl DbState {
    ...
    fn load_meta(&self, conn: &Connection) -> rusqlite::Result<TableMeta> {
        ...
        let fields: HashMap<String, FieldDef> = match meta_data.get("fields") {
            Some(s) => serde_json::from_str(s).unwrap_or_default(),
            None => HashMap::new(),
        };
    ...
    /*pub fn update_meta(&mut self, path: PathBuf, new_meta: TableMeta) -> Result<(), String> {
        ...
        self.with_conn(path, |conn, _| {
            ...
            let params = [
                ...
                ("fields", serde_json::to_value(&new_meta.fields)),
...
pub fn search_items(conn: &Connection, meta: &TableMeta, query: &str) -> Result<Vec<DataRecord>, String> {
    ...
    let search_fields = if meta.search_config.is_empty() {
        meta.fields.keys().cloned().collect::<Vec<String>>()
    } else {
...
pub fn add_field(
    ...
    conn.execute(
        ...
        params![serde_json::to_string(&fields).unwrap()],
    )?;
...
pub fn update_operation(conn: &Connection, operation: &Operation) -> Result<(), String> {
    ...
    if let Some(arr) = ops.as_array_mut() {
        if let Some(existing) = arr
            .iter_mut()
            .find(|op| op["id"].as_u64().unwrap_or(0) as u32 == operation.id)
        {
            *existing = json!({
                "id": operation.id,
                "name": operation.name,
...
pub fn create_example_refers(name: &str, path: &PathBuf) -> Result<(), String> {
    match name {
        ...
        _ => Err(
            RError::SqliteFailure(rusqlite::ffi::Error::new(1), Some("Unknown example refer ".to_string())).to_string(),
        ),
    }
}
...
Deposit";
    ...
    let info: Vec<(String, String)> = vec![
        ("name".to_string(), "Deposit".to_string()),
        ("desc".to_string(), "Calculate compound interest growth".to_string()),
    ];
...
Per 1 (base unit),1";
    ...
    let info: Vec<(String, String)> = vec![
        ("name".to_string(), "Shrinkflation".to_string()),
        ("desc".to_string(), "Compare prices per unit weight/volume".to_string()),
    ];
...
Dilution";
    ...
    let info: Vec<(String, String)> = vec![
        ("name".to_string(), "Dilution".to_string()),
        ("desc".to_string(), "Calculate solution mixing ratios".to_string()),
    ];
